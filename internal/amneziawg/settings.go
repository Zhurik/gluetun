...
func (s Settings) Validate() error {
	if s.JunkPacketMin > s.JunkPacketMax && s.JunkPacketMax != 0 {
		return fmt.Errorf("%w: jmin=%d and jmax=%d", ErrJunkPacketBounds,
			s.JunkPacketMin, s.JunkPacketMax)
	}
	...
	if s.JunkPacketCount == 0 && hasJunkSize {
		return fmt.Errorf("%w: jc=%d and jmin=%d and jmax=%d",
			ErrJunkPacketCount, s.JunkPacketCount, s.JunkPacketMin, s.JunkPacketMax)
	}

	if s.JunkPacketCount != 0 && (s.JunkPacketMin == 0 || s.JunkPacketMax == 0) {
		return fmt.Errorf("%w: jc=%d and jmin=%d and jmax=%d",
			ErrJunkPacketSize, s.JunkPacketCount, s.JunkPacketMin, s.JunkPacketMax)
	}
